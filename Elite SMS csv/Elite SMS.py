...
class Ui_MainWindow(object):
    ...
    def gotomain(self):
        ...
        self.macAddressToTest = ['20:e4:17:08:5c:07','20:0d:b0:17:77:67','08:2e:5f:11:51:fd','3e:4a:da:cd:42:6c']

        if self.connect1()==True:
            self.pcMacAddress = gma()
            if self.username=='faruk' and self.Password=='1234' and self.pcMacAddress in self.macAddressToTest:
                self.ui.setupUi(self.window)
                MainWindow.hide()
                self.window.show()
            elif not self.pcMacAddress in self.macAddressToTest:
                msg = QMessageBox()
                msg.setWindowTitle("ERROR")
                msg.setText("This Software can't run on your PC")
